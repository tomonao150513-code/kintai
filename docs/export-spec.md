...
| 4 | `休憩秒数` | 整数（`break_seconds`）※P7 で追加 | `0` |
| 5 | `実働秒数` | 整数（`duration_seconds` = 経過 − 休憩、丸めなし） | `5424` |
| 6 | `実働時間(HH:MM:SS)` | `format_hms_colon` | `1:30:24` |
| 7 | `実働時間(h)` | `format_hours_decimal`（小数2桁） | `1.51` |
| 8 | `プロジェクト` | プロジェクト名 | `社内システム開発` |
| 9 | `タスク` | タスク名 | `ログイン画面実装` |
| 10 | `タスク状態` | `未着手` / `進行中` / `完了`（表示ラベル） | `進行中` |
| 11 | `メモ` | `note`（改行はスペースに置換） | `詳細設計まで` |
| 12 | `入力元` | `打刻` / `手動` | `打刻` |
...
- 明細の最後に合計行（またはシート）: 日付〜終了時刻の列は空、`休憩秒数` / `実働秒数` = 総和、`実働時間(HH:MM:SS)` = 総和の整形、`実働時間(h)` = 総和/3600、以降は空。ラベルは `プロジェクト` 列に `合計`。
...
- 列の順序・ヘッダ名が仕様どおり（12 列）。
- BOM (`EF BB BF`) が先頭にある / 改行 CRLF。
- `実働秒数` が丸めなしで一致（休憩控除後）。
- 合計行の値が明細の総和と一致。
- 実行中記録が除外される。
- ファイル名に期間が入る（`kintai_{from}_{to}`）。
- xlsx が openpyxl で読み戻せて、数値セル（`休憩秒数` / `実働秒数` / `実働時間(h)`）が数値型。
- 0 件でもヘッダ + 合計行を返す。不正な期間はフォーム再表示（ダウンロードしない）。
